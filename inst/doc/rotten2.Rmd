...
mtab <- data.frame(
  ...
  model = c(
    "$Rotten_n \\sim \\mathcal{B}(inv_{logit}(\\beta_3*dbh_n))$",
    "$Rotten_n \\sim \\mathcal{B}(inv_{logit}({\\beta_0}_p + \\beta_3*dbh_n))$",
    "$Rotten_n \\sim \\mathcal{B}(inv_{logit}({\\beta_0}_p + {\\beta_1}_s + \\beta_3*dbh_n))$",
...
### Model Predictions.
```{r M pred}
beta <- as.matrix(fit)[which.max(as.matrix(fit)[,'lp__']),c('beta_3')]
Pr <- function(dbh, dbhmax, beta3){
  theta <- beta3*(dbh/dbhmax)
  inv_logit(theta)
}
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data2)[1]), function(i)
    Pr(data2$dbhest[i], dbhmax, x['beta_3'])
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95)))))
names(pred) <- c('5%', '95%')
pred$mean <- inv_logit((beta[1]/dbhmax)*data2$dbhest)
...
> $P_{rotten} = inv_{logit}(`r round(beta[1]/dbhmax,3)`*dbh)$
...
### Model Predictions.
```{r Mp pred}
beta <- as.matrix(fit)[which.max(as.matrix(fit)[,'lp__']),c('beta_0', 'beta_3')]
Pr <- function(dbh, dbhmax, beta0, beta3){
  theta <- beta0 + beta3*(dbh/dbhmax)
  inv_logit(theta)
}
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data2)[1]), function(i)
    Pr(data2$dbhest[i], dbhmax, x['beta_0'], x['beta_3'])
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95)))))
names(pred) <- c('5%', '95%')
...
# fit <- stan(file = file.path(mpath, 'Ms.p.stan'),
#             data = list(N = dim(data)[1],
#                         P = length(levels(as.factor(data$plot))),
#                         S = length(levels(as.factor(data$species))),
#                         plot = as.integer(as.factor(data$plot)),
...
#                         rotten = data$rotten),
#             chains = 1)
# save(fit, file = file.path(mpath, 'Ms.p.Rdata'))
load(file.path(mpath, 'Ms.p.Rdata'))
g <- fitgraph(fit)
```
