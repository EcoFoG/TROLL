...
```{r mtab}
mtab <- data.frame(
  m = c('$M$', '$M_{p}$', '$M_{p,s}$', '$M_{s,wsg,p}$', '$M_{wsg,p}$'),
  model = c(
...
kable(mtab)
row.names(mtab) <- c('M', 'Mp', 'Mps', 'Ms.wsg.p', 'Mwsg.p')
```
...
> $P_{rotten} = inv_{logit}(`r round(beta[1],3)` + `r round(beta[2],3)`*dbh)$
