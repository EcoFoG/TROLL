...
```{r mtab}
mtab <- data.frame(
  m = c('$M$', '$M_{p}$', '$M_{p,s}$', '$M_{p,s,w}$', '$M_{p,w}$'),
  model = c(
    ...
    "$Rotten_n \\sim \\mathcal{B}(inv_{logit}(\\beta_0 + \\beta_1*dbh_n + {\\beta_2}_p + {\\beta_3}_s + \\beta_4*dbh_n))$",
    "$Rotten_n \\sim \\mathcal{B}(inv_{logit}(\\beta_0 + \\beta_1*dbh_n + {\\beta_2}_p + \\beta_4*dbh_n))$"
    )
  )
mtab <- apply(mtab, 2, as.character)
kable(mtab)
row.names(mtab) <- c('M', 'Mp', 'Mps', 'Mpsw', 'Mpw')
```
...
### Model Predictions.
```{r M pred}
beta <- as.matrix(fit)[which.max(as.matrix(fit)[,'lp__']),c('beta[1]', 'beta[2]')]
...
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data2)[1]), function(i)
    Pr(data2$dbhest[i], dbhmax, x['beta[1]'], x['beta[2]'])
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95)))))
names(pred) <- c('5%', '95%')
pred$mean <- inv_logit(beta[1] + beta[2]*(data2$dbhest/dbhmax))
...
```{r Mpsw, include=FALSE}
# fit <- stan(file = file.path(mpath, 'Mpsw.stan'),
#             data = list(N = dim(data)[1],
#                         P = length(levels(as.factor(data$plot))),
#                         S = length(levels(as.factor(data$species))),
#                         plot = as.integer(as.factor(data$plot)),
...
#                         rotten = data$probed_rotten),
#             chains = 1)
# save(fit, file = file.path(mpath, 'Mpsw.Rdata'))
load(file.path(mpath, 'Mpsw.Rdata'))
g <- fitgraph(fit)
```
...
> `r paste(mtab['Mpsw',], collapse = ': ')`

### Model Predictions.
```{r Mpsw pred}
beta <- as.matrix(fit)[which.max(as.matrix(fit)[,'lp__']),c('beta_0', 'beta_1', 'beta_4')]
Pr <- function(dbh, dbhmax, wsg, wsgmax, beta0, beta1, beta4){
  theta <- beta0 + beta1*(wsg/wsgmax) + beta4*(dbh/dbhmax)
  inv_logit(theta)
}
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data2)[1]), function(i)
    Pr(data2$dbhest[i], dbhmax, data2$wsg[i], wsgmax, x['beta_0'], x['beta_1'], x['beta_4'])
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95)))))
names(pred) <- c('5%', '95%')
pred$mean <- inv_logit(beta[1] + (beta[2]/wsgmax)*data2$wsg + (beta[3]/dbhmax)*data2$dbhest)
pred$dbh <- data2$dbhest
pred$wsg <- data2$wsg
f <- ggplot(data2, aes(x = dbhest, y = probed_rotten)) +
  geom_ribbon(aes(x = pred$dbh, ymin = pred$`5%`, ymax = pred$`95%`), color = 'grey', alpha = 0.2) +
  geom_line(aes(x = pred$dbh, y = pred$`5%`, color = '5%'), linetype = 2) +
  geom_line(aes(x = pred$dbh, y = pred$`95%`, color = '95%'), linetype = 2) +
  geom_line(aes(x = pred$dbh, y = pred$mean, color = 'mean'), linetype = 2) +
  ...
  scale_color_manual(name = 'Model',
                     values = c('5%' = 'black',
                                '95%' = 'black',
                                'mean' = 'red'))
h <- ggplot(data2, aes(x = wsg, y = probed_rotten)) +
  ...
  geom_point() +
  theme_bw() +
  xlab('Wood specific gravity') +
  ylab('Probability to be probed as rotten') +
  scale_color_manual(name = 'Model',
                     values = c('5%' = 'black',
                                '95%' = 'black',
                                'mean' = 'red'))
cowplot::plot_grid(f,h, nrow = 2) ; rm(fit)
```
...
> $P_{rotten} = inv_{logit}(`r round(beta[1],3)` + `r round(beta[2]/wsgmax,3)`*wsg + `r round(beta[3]/dbhmax,3)`*dbh)$
...
```{r Mpw, include=FALSE}
# fit <- stan(file = file.path(mpath, 'Mpw.stan'),
#             data = list(N = dim(data)[1],
#                         P = length(levels(as.factor(data$plot))),
#                         plot = as.integer(as.factor(data$plot)),
#                         wsg = data$wsg/max(data$wsg),
...
#                         rotten = data$probed_rotten),
#             chains = 1)
# save(fit, file = file.path(mpath, 'Mpw.Rdata'))
load(file.path(mpath, 'Mpw.Rdata'))
g <- fitgraph(fit)
```
