...
datasub <- function(data, dataset){
  ...
  Nmax <- max(data$Nmass)
  if(dataset == 'full')
    dataset <- levels(droplevels(data$Dataset))
...
LLpred <- function(fit, data, one = TRUE){
  LMA <- data$LMA
  N <- data$Nmass
  LMAmax <- max(data$LMA)
  Nmax <- max(data$Nmass)
...
Maximum likekihood of **`r max(as.matrix(fit$full)[,'lp__'])`** and $R^2$ of **`r max(evaluate(fit, data))`**
...
Maximum likekihood of **`r max(as.matrix(fit$full)[,'lp__'])`** and $R^2$ of **`r max(evaluate(fit, data, one = FALSE))`**
...
fits <- list(fit1 = fit1, fit2 = fit2, fit3 = fit3, fit4 = fit4, 
             fit5 = fit5, fit6 = fit6, fit7 = fit7, fit8 = fit8, fit9 = fit9)
...
res <- data.frame(row.names = paste0('M', 1:9),
                  ...
                  R2 = NA)
res$R2[1:6] <- unlist(lapply(fits[1:6], function(fit) evaluate(fit, data)))
res$R2[2:9] <- unlist(lapply(fits[2:9], function(fit) evaluate(fit, data, one = FALSE)))
kable(t(res))
