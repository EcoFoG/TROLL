...
In order to study biodiversity effect on disturbance we decied to use @Loreau2001 partitionning. To do so we needed to generate a monoculture forest for each species present in community assemblages that we wanted to test. But monoculture simulations without disturbance over 500 years shown some strange results for species with low leaf mass per area (LMA). More specifically some monoculture show really low aboveground biomass (AGB) and basal area (BA, see figure 1) and really small maximal age and diameter at breast height (dbh) in the final stand (see figure 2).
...
Figure 2: Age, dbh maximum and Leaf Mass per Area (LMA) after 500 years. *Colour represents either the leaf lifespan present in the monoculture simulated with TROLL over 500 years or if maximum dbh is inferior to dmax.*
...
Currently abiotic environment is varying among month. What is interesting us is the minimal environement values for which tree with low $LMA$ will stop to assimilate carbohydrates. So for subsequent numeric applications we will use month with minimal maximum irradiance ($Wmax_{min}$), Januar with following maximum irradiance ($Wmax_{Jan}$), temperature ($T_{Jan}$) and VPD ($VPD_{Jan}$): `r Env[1,]`.
...
<span style="color:red"> Still PPFD vary relatively few in TROLL simulations (see table 1). And what is interesting us is minimal values that will play more on low LMA species. So for fsubsequent numeric applications we will use $PPFD_{min} = 1204$. </span>
...
Leaf assimilation $A$ is computed with following formulae inside TROLL:
...
Leaflifespan is currently computed from Reich allometry. We suggest to find a better value with @wright_worldwide_2004 GLOPNET dataset. First we can clearly see that allometry seems to greatly change between biomes (see figure 11).
...
g <- ggplot(data, aes(x = LMA, y = LL, colour = BIOME, label = Species))
g + geom_point() + geom_smooth(method = 'lm') + facet_wrap(~ BIOME) +
  scale_y_log10() + scale_x_log10() + ylab('log10 Leaf Lifespan (LL in months)') +
  xlab('log10 Leaf Mass per Area (LMA in g/m2)')
```

Figure 11: Leaf mass per area (LMA) and leaflifespan (LL). *Leaf mass per area (LMA in $g.m^{-2}$) and leaf lifespan (LL in $months$) are taken in GLOPTNET dataset from @wright_worldwide_2004 with a linear regression by biome.*
