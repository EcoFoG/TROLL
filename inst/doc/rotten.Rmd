...
Based on complexity (number of parameters), convergence and likelihood we selected model $M_{p,s}$ (see see [rotten 2](rotten2.html)):

`r paste(mtab['Mps',], collapse = ': ')`

```{r M model, include=FALSE}
...
ggplot(data2, aes(x = dbhest, y = probed_rotten)) +
  ...
  scale_color_manual(name = 'Model',
                     values = c('5%' = 'black',
                                ...
                                'mean' = 'red'))
```
Figure 1: Models prediction. *Probability to be probed as rotten P for a given dbh (cm).*

$P_{rotten} = inv_{logit}(`r round(beta[1],3)` + `r round(beta[2]/dbhmax,3)`*dbh)$

Table 2: Models prediction. *Probability to be probed as rotten (P in %) for a given dbh (cm).*
```{r M pred}
...
Based on complexity (number of parameters), convergence and likelihood we selected model $N_{p,s}$ associated to hyperparameter $\rho$ with model $L_{p,s}$ (see see [rotten 3](rotten3.html)):

`r paste(mtab['Nps',], collapse = ': ')`

```{r N model, include=FALSE}
data <- read_csv("~/Documents/ECOFOG/TROLL/inst/extdata/Rotten/rotten_data.csv")
data <- data[c('num', 'Parcelle', 'up', 'ess', 'abattu', 'tout BE', 'sonde_creux', 'sonde_creux abatt', 'sonde_creux_refus', 'DBH_desi', 'V_Est', 'vol_BO')]
names(data) <- c('id', 'plot', 'subplot', 'spCode', 'logged', 'rotten_full', 'probed_rotten', 'probed_rotten_logged', 'probed_rotten_refused', 'dbhest', 'Vest', 'Vbo')
...
ggplot(data2, aes(x = dbhest, y = Vbo)) +
  ...
  scale_color_manual(name = 'Model',
                     values = c('5%' = 'black',
                                ...
                                'mean' = 'red'))
```
Figure 2: Models prediction. *Final volume of wood for a given dbh (cm).*

> $Volume_{of~wood} = (`r round(pars[1]/dbhmax^2,5)`*dbh^2)*[1-Pr*(`r round(pars[3]/dbhmax^2,5)`*dbh^2)]$

Table 3: Models prediction. *Final volume of wood ($V_f$ in $m^3$) and percent of rotten wood ($V_p$ in %) for a given dbh (cm) if the tree was probed rotten.*
```{r N pred}
table <- data.frame(row.names = seq(45, 125, 5),
...
We will use model M of probability to probed as rotten inside TROLL model in order to get a selective logging closer to reality.
