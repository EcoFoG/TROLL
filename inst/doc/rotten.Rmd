...
data <- group_by(data, dbhest) %>%
  summarise_each(funs(mean))
Pr <- function(dbh, dbhmax, beta0, beta3){
  theta <- beta0 + (beta3/dbhmax)*dbh
  gtools::inv.logit(theta)
}
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data)[1]), function(i)
    Pr(data$dbhest[i], dbhmax, x['beta_0'], x['beta_3'])
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95)))))
names(pred) <- c('5%', '95%')
pred$mean <- gtools::inv.logit(beta[1] + (beta[2]/dbhmax)*data$dbhest)
pred$dbh <- data$dbhest
(g <- ggplot(data, aes(x = dbhest, y = probed_rotten)) +
