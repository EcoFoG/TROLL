...
```{r DOE}
data <- data.frame(
  Name = as.vector(sapply(rich, function(r) paste(r, 1:10, sep = '.'))),
  Richness = as.vector(mapply(function(x,y) rep(x,y), x = rich, y = head(unlist(lapply(Sel, length)),4))),
  Hypervolume = unlist(lapply(Sample, function(x) x[,2])),
  Dist = unlist(lapply(Sample, function(x) x[,1])))
ggplot(data, aes(x = Hypervolume, y = Dist, label = Name)) +
  geom_line() + geom_point() + geom_label() +
  facet_wrap(~ Richness, scales = "free") +
  ylab('Euclidean distance to metacommunity centroïd')
```
