...
To get more robust results we decided to filter species data with more robust trait values. To do so we assessed the mean number of observation needed to get a trait value the closest to the trait mean value.

```{r Bridge, fig.height=4}
species <- read.table('~/Documents/ECOFOG/TROLL/inst/extdata/species.txt',
                      header=TRUE, dec=".", sep="", row.names = 1)[c('LMA', 'wsg')]
data <- data.table::fread('~/Documents/ECOFOG/TROLL/inst/extdata/BridgeDATA.csv',
                          data.table = F)
data$name <- paste0(data$Genus, '_', data$species)
data <- data[data$name %in% row.names(species),]
data$wsg <- data$sapwood_dens
data$LMA <- data$dry_mass / data$ind_surf_area
data <- data[c('name', 'LMA', 'wsg')]
data$LMA <- data$LMA * 10^5
datasum <- data.frame(apply(data[c('LMA', 'wsg')], 2,
                      function(x) tapply(x, data$name, mean, na.rm = T)))
datasum$N <- table(data$name)
datasum <- cbind(datasum,
                 data.frame(apply(data[c('LMA', 'wsg')], 2,
                                  function(x) tapply(x, data$name, var, na.rm = T))))
names(datasum) <- c('LMAm', 'wsgm', 'N', 'LMAv', 'wsgv')
species$sp <- row.names(species)
species$N <- as.vector(datasum$N[match(species$sp, row.names(datasum))])
species$five <- (species$N < 5)
species$label <- paste(species$sp, species$N)
ggplot(species, aes(x = LMA, y = wsg, color = five, label = label)) + geom_point()
sprm <- species[species$five,c('sp', 'N')]
sprm <- sprm[-which(is.na(sprm$sp)),]
sprm <- paste(apply(sprm, 1, function(x) 
  paste0(x[1], ' (n=', x[2], ')')), collapse = ', ')
rm(species, data, datasum)
```

Removed species being: `r sprm`.
...
# # Init
# seedrain <- 50
# dir.create(file.path(path, 'maturity2'))
# invisible(sapply(sim, function(s) dir.create(file.path(path, 'maturity2', s))))
# species <- read.table('~/Documents/ECOFOG/TROLL/inst/extdata/species.txt',
#                       header=TRUE, dec=".", sep="")
...
#   init(path = file.path(path, 'maturity2'),
#        input = paste0(s, '.txt'),
#        species = Sel[[s]],
#        numesp = dim(Sel[[s]])[1],
...
# }))
# # Bash
# if('maturity.sh' %in% list.files(file.path(path, 'maturity2')))
#    unlink(file.path(path, 'maturity2', 'maturity.sh'))
# bash <- file(file.path(path, 'maturity2', 'maturity.sh'), open = 'a')
...
# Load stack
# mature <- loadStack('~/Documents/ECOFOG/Results/disturbance/maturity2/model/', old = T)
# save(mature, file = '~/Documents/ECOFOG/Results/disturbance/maturity2/mature.Rdata')
load('~/Documents/ECOFOG/Results/disturbance/maturity2/mature.Rdata')
...
# Load stack 
# mm <- loadStack('~/Documents/ECOFOG/Results/disturbance/monoculture/mature/Run1/Model', old = T)
# save(mm, file = '~/Documents/ECOFOG/Results/disturbance/monoculture/mature/Run1/Run1.Rdata')
...
g2 <- ggplot(data, aes(x = n10, y = n30, colour = sim, label = traits)) +
  geom_point() + guides(colour = F)
(p1 <- plotly_POST(ggplotly(g1), filename="p1"))
(p2 <- plotly_POST(ggplotly(g2), filename="p2"))
