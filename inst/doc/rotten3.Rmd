...
mtab <- data.frame(
  N = c('$N_{p+s}$'),
  Model = c(
    "$V_f \\sim \\mathcal{N} ({\\beta_0}_p + {\\beta_1}_s + \\beta_2*dbh - V_r*P_r, \\sigma)$"
    )
  )
mtab$N <- as.character(mtab$N)
mtab$Model <- as.character(mtab$Model)
kable(mtab)
row.names(mtab) <- c('Np+s')
```
