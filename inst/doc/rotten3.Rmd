...
```{r mtab2}
mtab2 <- data.frame(
  L = c('$L$', '$L_d$', '$L_{p}$', '$L_{s}$', '$L_{p,s}$'),
  Model = c(
    "$\\rho = \\theta + \\theta_d*dbh^2$",
    "$\\rho = \\theta *dbh^2$",
    "$\\rho = (\\theta + \\theta_p) *dbh^2$",
    "$\\rho = (\\theta + \\theta_s) *dbh^2$",
    "$\\rho = (\\theta + \\theta_p + \\theta_s) *dbh^2$"
    )
  )
mtab2$L <- as.character(mtab2$L)
mtab2$Model <- as.character(mtab2$Model)
row.names(mtab2) <- c('L', 'Ld', 'Lp', 'Ls', 'Lps')
...
> `r paste(mtab2['L',], collapse = ': ')`

### Model Predictions.
```{r L pred}
pars <- as.matrix(fit)[which.max(as.matrix(fit)[,'lp__']),c('beta', 'sigma', 'theta', 'theta_d')]
Vf <- function(dbh, dbhmax, Pr, pars){
  rho <- pars[3] + pars[4]*(dbh/dbhmax)*(dbh/dbhmax)
  mu <- (pars[1]*(dbh/dbhmax)*(dbh/dbhmax))*(1-Pr*rho)
  rlnorm(1, log(mu), pars[2])
}
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data2)[1]), function(i)
    Vf(data2$dbhest[i], dbhmax, data2$probed_rotten[i], c(x['beta'], x['sigma'], x['theta'], x['theta_d']))
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95), na.rm = T))))
names(pred) <- c('5%', '95%')
pred$mean <- (pars[1]*(data2$dbhest/dbhmax)*(data2$dbhest/dbhmax))*(1-data2$probed_rotten*(pars[3] + pars[4]*(data2$dbhest/dbhmax)*(data2$dbhest/dbhmax)))
pred$dbh <- data2$dbhest
ggplot(data2, aes(x = dbhest, y = Vbo)) +
  geom_ribbon(aes(x = pred$dbh, ymin = pred$`5%`, ymax = pred$`95%`), color = 'grey', alpha = 0.2) +
  geom_line(aes(x = pred$dbh, y = pred$`5%`, color = '5%'), linetype = 2) +
...
> $P_{rotten} = (`r round(pars[1],3)`*dbh^2)*[1-Pr*(`r round(pars[3],3)` + `r round(pars[4],3)`*dbh^2)]$
...
```{r Ld, include=FALSE}
# fit <- stan(file = file.path(mpath, 'Ld.stan'),
#             data = list(N = dim(data)[1],
#                         dbh = data$dbhest/max(data$dbhest),
#                         Pr = data$probed_rotten,
#                         Vf = data$Vbo,
...
#                         species = as.integer(as.factor(data$species))),
#             chains = 1)
# save(fit, file = file.path(mpath, 'Ld.Rdata'))
load(file.path(mpath, 'Ld.Rdata'))
g <- fitgraph(fit, rho = F)
```
...
> $P_{rotten} = (`r round(pars[1],3)`*dbh^2)*[1-Pr*(`r round(pars[3],3)`*dbh^2)]$
...
```{r Lp, include=FALSE}
# fit <- stan(file = file.path(mpath, 'Lp.stan'),
#             data = list(N = dim(data)[1],
#                         dbh = data$dbhest/max(data$dbhest),
#                         Pr = data$probed_rotten,
#                         Vf = data$Vbo,
...
#                         species = as.integer(as.factor(data$species))),
#             chains = 1)
# save(fit, file = file.path(mpath, 'Lp.Rdata'))
load(file.path(mpath, 'Lp.Rdata'))
g <- fitgraph(fit, rho = F)
```
...
```{r Ls, include=FALSE}
# fit <- stan(file = file.path(mpath, 'Ls.stan'),
#             data = list(N = dim(data)[1],
#                         dbh = data$dbhest/max(data$dbhest),
#                         Pr = data$probed_rotten,
#                         Vf = data$Vbo,
...
#                         species = as.integer(as.factor(data$species))),
#             chains = 1)
# save(fit, file = file.path(mpath, 'Ls.Rdata'))
load(file.path(mpath, 'Ls.Rdata'))
g <- fitgraph(fit, rho = F)
```
...
```{r Lps, include=FALSE}
# fit <- stan(file = file.path(mpath, 'Lps.stan'),
#             data = list(N = dim(data)[1],
#                         dbh = data$dbhest/max(data$dbhest),
#                         Pr = data$probed_rotten,
#                         Vf = data$Vbo,
...
#                         species = as.integer(as.factor(data$species))),
#             chains = 1)
# save(fit, file = file.path(mpath, 'Lps.Rdata'))
...
> Adding random effect for plot and/or species ($\theta_p$, $\theta_s$) increased likelihood and decreased model variance ($\sigma$). Consequently best model is $N_{p,s}$ associated to $L_{p,s}$: $$V_f \sim \mathcal{logN} (log[((\beta + \beta_p + \beta_s)*dbh^2)*(1 - Pr*\rho)], \sigma)$$ $$\rho = (\theta +  \theta_p + \theta_s) *dbh^2$$
