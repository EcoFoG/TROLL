...
# HV <- lapply(coms, function(x)  lapply(x, function(y) 
#     try(get_volume(hypervolume(scale(y), bandwidth = bd, verbose = F, warnings = F)))))
# HV <- lapply(HV, function(x){x[which(unlist(lapply(x, function(y) inherits(y, 'try-error'))))] <- NA})
# HV <- data.frame(do.call('cbind', lapply(coms, unlist)))
# names(HV) <- paste0('HV.', rich) ; rm(bd)
# # Sample
# Sample <- cbind(Dist, HV)
...
# names(Sel) <- rich ; rm(Scale, HV, Dist)
