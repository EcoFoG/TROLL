...
data$alive <- as.integer(data$status2 == 'alive')
dgmax <- max(data$gaps)
...
mtab <- data.frame(
  ...
  Model = c(
    ...
    '$M \\sim \\mathcal{B}(inv_{logit}[\\theta + \\beta*e^{-\\alpha*d_{gaps}}])$',
    '$M \\sim \\mathcal{B}(inv_{logit}[\\theta + S*\\beta*e^{-\\alpha*d_{gaps}}])$',
    '$M \\sim \\mathcal{B}(inv_{logit}[\\theta + \\beta*e^{-\\alpha*d_{disturbed~area}}])$'
    )
)
...
### Model Predictions.
```{r Mg pred}
pars <- as.matrix(fit)[which.max(as.matrix(fit)[,'lp__']),c('theta', 'beta', 'alpha')]
P <- function(dgaps, dgmax, pars) inv_logit(pars[1] + pars[2]*exp(-pars[3]*dgaps))
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data2)[1]), function(i)
    P(data2$gaps[i], dgmax, c(x['theta'], x['beta'], x['alpha']))
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95)))))
names(pred) <- c('5%', '95%')
pred$mean <- inv_logit(pars[1] + pars[2]*exp(-pars[3]*data2$gaps))
pred$gaps <- data2$gaps
ggplot(data2, aes(x = gaps)) +
  geom_ribbon(aes(x = pred$gaps, ymin = pred$`5%`, ymax = pred$`95%`), color = 'grey', alpha = 0.2) +
  geom_line(aes(x = pred$gaps, y = pred$`5%`, color = '5%'), linetype = 2) +
...
> $P_{M} = inv_{logit}(`r round(pars[1],3)` + `r round(pars[2],3)`*e^{-`r round(pars[3],3)`*d_{gaps}})$
...
pars <- as.matrix(fit)[which.max(as.matrix(fit)[,'lp__']),c('theta', 'beta', 'alpha')]
P <- function(dgaps, dgmax, S, pars) inv_logit(pars[1] + S*pars[2]*exp(-pars[3]*dgaps))
pred <- apply(as.matrix(fit), 1, function(x)
  sapply(seq_len(dim(data2)[1]), function(i)
    P(data2$gaps[i], dgmax, data2$S[i], c(x['theta'], x['beta'], x['alpha']))
  ))
pred <- data.frame(t(apply(pred, 1, function(x) quantile(x, probs = c(0.05, 0.95)))))
names(pred) <- c('5%', '95%')
pred$mean <- inv_logit(pars[1] + data2$S*pars[2]*exp(-pars[3]*data2$gaps))
pred$gaps <- data2$gaps
ggplot(data2, aes(x = gaps)) +
  geom_ribbon(aes(x = pred$gaps, ymin = pred$`5%`, ymax = pred$`95%`), color = 'grey', alpha = 0.2) +
  geom_line(aes(x = pred$gaps, y = pred$`5%`, color = '5%'), linetype = 2) +
