...
model {
   real f[N] ;
   for(n in 1:N){
     f[n] = beta_0s[J[n]] * exp(beta_1s[J[n]]*LMA[n] - beta_2s[J[n]]*Nmass[n]) ;
   }
...
pars <- c("sigma", "beta_0", "sigma_0", "beta_1", "sigma_1", "beta_2", "sigma_2", "lp__")
...
beta_1 <- pars['beta_1']
beta_2 <- pars['beta_2']
LLpred <- function(LMA, Nmass, LMA_max, Nmass_max, beta_0, beta_1, beta_2){
  fact <- beta_1*(LMA/LMA_max) - beta_2*(Nmass/Nmass_max)
...
pred$LL <- sapply(pred$LMA, function(x) 
  sapply(pred$Nmass, function(y) 
    LLpred(x, y, LMA_max, Nmass_max, beta_0, beta_1, beta_2)))
...
$$LL = `r round(beta_0,3)`*e^{`r round(beta_1,3)`*LMA - `r round(beta_2,3)`*Nmass}$$
