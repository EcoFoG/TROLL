...
model {
   real f[N] ;
   for(n in 1:N){
     f[n] = exp(beta_1s[J[n]]*pow(LMA[n],beta_3s[J[n]]) - beta_2s[J[n]]*pow(Nmass[n],beta_4s[J[n]])) ;
   }
...
pars <- c("sigma", "beta_1", "sigma_1", "beta_2", "sigma_2", "beta_3", "sigma_3", "beta_4", "sigma_4", "lp__")
...
beta_3 <- pars['beta_3']
beta_4 <- pars['beta_4']
LLpred <- function(LMA, Nmass, LMA_max, Nmass_max,beta_1, beta_2, beta_3, beta_4){
...
pred$LL <- sapply(pred$LMA, function(x)
  sapply(pred$Nmass, function(y)
    LLpred(x, y, LMA_max, Nmass_max, beta_1, beta_2, beta_3, beta_4)))
...
$$LL = e^{`r round(beta_1/(LMA_max^beta_3),3)` *LMA^{`r round(beta_3,3)`} -`r round(beta_2/(Nmass_max^beta_3),3)`*Nmass^{`r round(beta_4,3)`} }$$
