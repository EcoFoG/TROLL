...
We then compiled GLOPNET, TRY, and DRYAD database to gather the biggest dataset. Dataset origin conserved for species was the one of the leaf lifespan observed in GLOPNET corresponding also to LMA and Nmass in most of cases. We measured variable importance in functionnal traits to explain leaf lifespan with out-of bag method applied on a random forest (see Table 2). We then sampled both model **M1** and a new model **M10** with new variables highlighted by previous test. We removed dataset random effect on $\beta_3$ due to the fact that all wsg were provided by DRYAD database. Due to a correlation between $\beta_0$ and $\beta_3$, we tried also a model **M11** similar to **M10** without $\beta_2$. 
...
Parameters pairs plot for model M10. *Parameters density distribution, pairs plot and Pearson's coefficient of correlation.*

```{r p11, fig.width=12}
g11$p
```

Parameters pairs plot for model M11. *Parameters density distribution, pairs plot and Pearson's coefficient of correlation.*

```{r graph10, fig.height=12, fig.width=12}
...
Figure 4: Leaflifespan predictions for model **M11** with leaf mass per area (LMA), leaf nitrogen content (Nmass), and wood specific gravity (wsg). *Leaf lifespan (LL in $months$) is predicted with model M10 fit. Leaf mass per area (LMA in $g.m^{-2}$) and leaf nitrogen content (Nmass, in %), and wood specific gravity (wsg in $mg.mm^{-3}$) are taken in GLOPNET dataset from @wright_worldwide_2004.* **Warning** *LMA (resp. Nmass and wsg) is not constant and depend on the closest point value for right (resp. center and left) graph.* 
...
Figure 5: Leaflifespan predictions for model different allometries with leaf mass per area (LMA), and leaf nitrogen content (Nmass), and wood specific gravity (wsg). *Leaf lifespan (LL in $months$) is predicted with model M1, M10, and M11 fits or Reich's and Wright's allometries [@Reich1991a]. Leaf mass per area (LMA in $g.m^{-2}$) leaf nitrogen content (Nmass, in %) , and wood specific gravity (wsg in $mg.mm^{-3}$) are taken in BRIDGE dataset used by TROLL [@Li].*
