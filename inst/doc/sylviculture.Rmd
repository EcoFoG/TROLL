...
On the other in current implementation a tree die and fall because he has been injured. This process is simulated by the variable $hurt$. The probability to die follows formula:
...
$$P(Death_i) = \frac{Height}{2*hurt} ~~ or ~~ hurt = \frac{Height}{2*P(Death_i)}=\frac{Height}{2*logit^{-1}(\theta + \beta*e^{\alpha*d_{gaps}})}$$
$$hurt = \frac{Height}{2*logit^{-1}(-4.441 + 0.762*e^{0.064*d_{gaps}})}$$
...
- Trees choice
    ...
    - Rotten: tree are considered as rotten depending on their diameter following our allometry
- Harvesting
    ...
    - Other damages: tree are hurted depending on their distance to a logging gap following our allometry
